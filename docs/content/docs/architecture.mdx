...
+---------------------------------------------------------------+
|                           HEADROOM                            |
|    Proxy (Rust)   ·   Python compress()   ·   TS compress()         |
|                               |                               |
|                               v                               |
...
In proxy mode the server is a Rust binary (`headroom-proxy`) with per-provider handlers (Anthropic, OpenAI, Gemini, Bedrock) that each run the same compression pipeline before forwarding through the selected [backend](/docs/proxy#cloud-providers).
