...
<Callout type="info">
The proxy's old `--backend vertex_ai` / `bedrock` LiteLLM routing was retired
(PR-3.2): the Rust proxy speaks native Bedrock/Vertex envelopes. The callback
documented here is the supported way to use LiteLLM with Headroom.
</Callout>
